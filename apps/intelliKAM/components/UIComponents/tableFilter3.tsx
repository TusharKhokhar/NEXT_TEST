...
const customStyles = {
  ...
  pageLink:"bg-white hover:bg-black hover:text-white text-black px-4 py-2 border border-gray-300 rounded-md px-4 py-2 focus:ring-1 focus:ring-black cursor-pointer",
...
const TableComponents3: React.FC<TableProps> = () => {
  ...
  let newData: any = []
  const filterFirstNameSearch = () => {
    if (!searchFirstname) {
      setfilteredData(allData)
    } else {
      searchFirstname.split(', ')
      newData = filteredData?.filter(function (obj: any) {
        return searchFirstname.includes(obj.FirstName)
      })
      setfilteredData(newData)
    }
  }

  const filterIndustrySearch = () => {
    if (!searchIndustry) {
      setfilteredData(allData)
    } else {
      searchIndustry.split(', ')
      newData = contacts?.filter(function (obj: any) {
        return searchIndustry.includes(obj.Industry)
      })
      setfilteredData(newData)
    }
  }
  const filterJobTitleSearch = () => {
    if (!searchJobTitle) {
      setfilteredData(allData)
    } else {
      searchJobTitle.split(', ')
      newData = contacts?.filter(function (obj: any) {
        return searchJobTitle.includes(obj.Title)
      })
      setfilteredData(newData)
    }
  }
  const filterWebsiteSearch = () => {
    if (!searchWebsite) {
      setfilteredData(allData)
    } else {
      searchWebsite.split(', ')
      newData = contacts?.filter(function (obj: any) {
        return searchWebsite.includes(obj.Website)
      })
      setfilteredData(newData)
    }
  }
  const filterCompanySearch = () => {
    if (!searchCompany) {
      setfilteredData(allData)
    } else {
      searchCompany.split(', ')
      newData = contacts?.filter(function (obj: any) {
        return searchCompany.includes(obj.Company)
      })
      setfilteredData(newData)
    }
  }
  const filterEmailSearch = () => {
    if (!searchEmail) {
      setfilteredData(allData)
    } else {
      searchEmail.split(', ')
      newData = contacts?.filter(function (obj: any) {
        return searchEmail.includes(obj.Email)
      })
      setfilteredData(newData)
    }
  }
  const filterCountrySearch = () => {
    if (!searchCountry) {
      setfilteredData(allData)
    } else {
      searchCountry.split(', ')
      newData = contacts?.filter(function (obj: any) {
        return searchCountry.includes(obj.Country)
      })
      setfilteredData(newData)
    }
  }
  const filterPhoneSearch = () => {
    if (!searchPhone) {
      setfilteredData(allData)
    } else {
      searchPhone.split(', ')
      newData = contacts?.filter(function (obj: any) {
        return searchPhone.includes(obj.Phone)
      })
      setfilteredData(newData)
    }
  }
  ...
  const handleImportContacts = async () => {
    // const contactsData: any = await axios.post(`/api/salesforce/leads`)
  ...
  return (
    ...
    //     className="saleforce-table ml-5"
    //     >
    //       <ReactPaginate
...
export default TableComponents3
